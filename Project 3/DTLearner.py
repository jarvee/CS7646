...
class DTLearner(object):
    ...
    def build_tree(self, data):
        ...
        if np.std(data[:,-1]) == 0:
            ...
            best_Value = np.corrcoef(data, rowvar = False)
            best_result = best_Value[0:-1,-1]
            best_Value_result = abs(best_result)
            if np.std(best_Value_result) == 0:
                i = random.randint(0,len(best_Value_result)-1)
            else:
                i = np.argmax(best_Value_result)
